...
| Data Len    | For variable-length fields. How many numbers defines the field own length. Should ba take from E-pay specification document. In the specification usualy marks as `LLVAR` - 2 digits, `LLLVAR` - 3 digits, and so on¹   |                                                              | Number, min value is 0           | Yes          |
| Tag Len     | For complex fields only (field should contain subfields). How many numbers defines the subfield length. In the specification usually marks as `LLVAR` - 2 digits, `LLLVAR` - 3 digits, and so on¹                       | -                                                            | Number, min value is 0           | Yes          |
...
| Secret      | If yes field value will be hidden in the log, transaction constructor, and on MainWindow display³                                                                                                                       | Display data, log writting                                   | Checkbox                         | Yes          |

¹ See more about variable length settings in corresponding chapter

² Currently SvTerminal supports random generated fields values. No date-time and other specific format are supported

³ Due to security reasons it is impossible to set Primary Account Number (Field 2) as non-secret. The field has non-removable "secret" mark  
